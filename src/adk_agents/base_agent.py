...
from a2a.types import Agent as A2AAgent, Task, TaskStatus
...
class AgentADK(ProtocolAgent):
    def __init__(self, name: str, instruction: str, model: str = None, tools: list = None):
        ...
        self.agent = Agent(model=LiteLlm(model=model_id), name=name, instruction=instruction, tools=self.tools)

    @property
    def name(self) -> str: return self._name

    @property
    def description(self) -> str: return self._instruction

    async def process_async(self, inputs: Dict[str, Any]) -> AgentResponse:
        return AgentResponse(id="temp-id", timestamp="iso-time", source_agent=self.name, target_agent="NextAgent", message_type="RESPONSE", payload={})
